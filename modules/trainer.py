...
import time, copy, os, re, cv2, random
...
class IOUMetric(object):
    ...
    def evaluate(self):
        ...
        self.sub_metric["classes_acc"] = mean_acc[-1]
        self.sub_metric["classes_recall"] = mean_recall[-1]
        self.sub_metric["classes_f1"] = mean_f1[-1]
        ...
        self.sub_metric["classes_iou"] = iou[-1]
...
class BaseMetricHandle(object):
    ...
    def get_sub_metric(self, pred_y, real_y, flag=0):
        ...
        # 误检率
        fun = lambda x, y: 1.0 * max(1.0, len(np.where(x == 1)[0]) - len(np.where((x == 1) & (y == 1))[0])) / max(1.0,
                                                                                                                  len(
                                                                                                                      np.where(
                                                                                                                          x == 1)[
                                                                                                                          0]))
        ...
        # 漏检率
        fun = lambda x, y: 1.0 * max(1.0, len(np.where(y == 1)[0]) - len(np.where((x == 1) & (y == 1))[0])) / max(1.0,
                                                                                                                  len(
                                                                                                                      np.where(
                                                                                                                          y == 1)[
                                                                                                                          0]))
        ...
        # F1-score
        fun = lambda x, y: 2.0 * max(1.0, len(np.where((x == 1) & (y == 1))[0])) / max(1.0,
                                                                                       len(np.where(x == 1)[0]) + len(
                                                                                           np.where(y == 1)[0]))
    ...
    def epoch_step(self, loader, forward_fun, prefix=""):
        metric = dict()
        keys = ["acc", "mean_acc", "classes_acc", "classes_recall", "mean_recall",
                "classes_iou", "mean_iou", "classes_f1", "mean_f1", "d_loss", "g_loss", "loss"]
        ...
        self.iou_metric.init()  #每一个epoch后将之前统计的结果清空
        for key in keys:
            metric["{}_{}".format(prefix, key)] += sub_metric.get(key, 0.0)
        return metric
    ...
    def tensorboard_add_imgs(self, model, test_loader):
        model.eval()
        for i, data in enumerate(test_loader, 1):
            ...
            x = data_x.data.cpu().numpy()
            y = data_y.data.cpu().numpy()
            pred = pred.data.cpu().numpy()
            id = random.randint(0, len(x) - 1)
            x, y, pred = x[id], y[id], pred[id]
            img1 = np.transpose((x[:3, :, :] * 255), (1, 2, 0)).astype(int)
            img2 = np.transpose((x[3:, :, :] * 255), (1, 2, 0)).astype(int)
            gt = (y * 255).astype(int)
            pred = (pred * 255).astype(int)
            self.board_writer.add_image("img1", img1)
            self.board_writer.add_image("img2", img2)
            self.board_writer.add_image("gt", gt)
            self.board_writer.add_image("pred", pred)
...
class BasicGanTrainer(BaseMetricHandle):
    ...
    def __init__(self, conf, model_path=None):
        ...
        self.use_gpu = conf.USE_GPU & torch.cuda.is_available()  # 判断是否有GPU加速
        ...
        if self.use_gpu:
            ...
            self.gpus = -1  # 使用gpu id的list
            self.default_device_id = -1
...
class BasicNNTrainer(BaseMetricHandle):
    ...
    def train(self, train_loader, vali_loader):
        ...
        for epoch in range(self.conf.TRAIN.MAX_EPOCHS):
            ...
            for board_key in vali_metric.keys():
                self.board_writer.add_scalar(board_key, vali_metric.get(board_key), epoch)
            # self.tensorboard_add_imgs(self.model, vali_loader)
            metric_path = os.path.join(self.model_path, 'metric.json')
            store_json(metric_path, log_data)
        self.board_writer.close()
        self.best_model_path = self.model_path
...
class BasicSSGanTrainer(BaseMetricHandle):
    ...
    def train_step(self, data, metric, batch_id):
        ...
        # backward
        gp_loss = self.calc_gradient_penalty(self.D, labeled_x, G_fake_out.detach())
        D_train_loss = D_real_loss + 0.1 * D_fake_loss + 0.05 * D_unlabeled_loss + gp_loss
        ...
        self.iou_metric.add_batch(pred.data.cpu().numpy(), real_y.data.cpu().numpy())
        return metric
    ...
    def predict_step(self, data, metric, batch_id):
        ...
        indices = torch.LongTensor([0, 1])
        if self.use_gpu: indices = indices.cuda()
        D_out = torch.index_select(D_out, 1, indices)
        _, pred = torch.max(D_out, 1)
        self.iou_metric.add_batch(pred.data.cpu().numpy(), data_y.data.cpu().numpy())
        # sub_metric = self.get_sub_metric(pred.data.cpu().numpy(), data_y.data.cpu().numpy())
        # sample_png_fule_name = os.path.join(self.sample_path, 'predict_samples_%.3d.jpg' % batch_id)
        # save_image(self.denorm(G_out.data), sample_png_fule_name, nrow=4, padding=2)
    ...
    def predict(self, test_loader):
        ...
        st = time.time()
        test_metric = self.epoch_step(test_loader, self.predict_step, prefix="test")
        print("time use %.4d  test_metric"%(time.time()-st), test_metric)
